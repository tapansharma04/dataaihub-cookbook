...
class InMemoryVectorStore:
    ...
    Enough for teaching retrieval. Not durable, not distributed.
    """
    ...
    def search(self, query_embedding: list[float], top_k: int = 3) -> list[ScoredChunk]:
        ...
        # argpartition then sort the top-k for stable ranked output
        candidate_idx = np.argpartition(-scores, kth=k - 1)[:k]
        ranked = candidate_idx[np.argsort(-scores[candidate_idx])]

        return [
            ScoredChunk(chunk=self._chunks[i], score=float(scores[i])) for i in ranked
        ]
