"""mcp-resources — MCP initialize → resources/list → resources/read lifecycle.

Example ID: mcp-resources

Protocol lifecycle:
  Client → initialize → MCP Server → resources/list → resources/read → content
"""
...
def _print_run(result: McpRunResult, *, show_sequence: bool) -> None:
    ...
    print(
        ...
        f"read={result.metrics.resource_read_ms}ms "
        f"resources={result.metrics.resources_discovered} "
        f"reads={result.metrics.resources_read} "
        f"(ok={result.metrics.successful_reads} "
        f"fail={result.metrics.failed_reads}) "
        f"bytes={result.metrics.resource_bytes}"
    )
    if result.discovered_resources:
        uris = ", ".join(resource.uri for resource in result.discovered_resources)
        print(f"Resources: {uris}")
    for read in result.output.get("reads", []):
        if read.get("isError"):
            print(f"Read:     {read['requestedUri']} REJECTED")
        else:
            print(
                f"Read:     requested={read['requestedUri']} "
                f"returned={read.get('returnedUri')} "
                f"mime={read.get('mimeType')}"
            )
    if show_sequence:
        print("\nSequence:")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--case",
        choices=case_ids,
        help="Run a measured MCP resource protocol case",
    )
    parser.add_argument(
        "--list-cases",
        action="store_true",
