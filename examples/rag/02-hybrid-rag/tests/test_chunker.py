...
def test_chunk_text_produces_one_chunk_per_section():
    ...
    # Error code section should be separable from general connectivity prose.
    code_chunks = [c for c in chunks if "E_CONN_42" in c.text]
    prose_chunks = [
...
def test_sample_contains_lexical_anchors():
    text = load_document(SAMPLE)
    assert "E_CONN_42" in text
    assert "NebulaAPI v2.3" in text
