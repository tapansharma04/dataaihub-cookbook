"""One-shot: run reranking and emit guided Lab traces as JSON (measured timings)."""
...
from rag.reranker import CrossEncoderReranker, rank_movement
from rag.store import InMemoryVectorStore

# Selected after measured runs — see README acceptance notes.
QUESTIONS = [
    {
        "traceId": "clear-reordering-cert-vs-econn42",
        "question": "Is calendar certificate rotation the fix for E_CONN_42?",
    },
    {
        "traceId": "ranking-agrees-handshake-checklist",
        "question": (
            "Intermittent TLS handshake failures without a stable error code "
            "— checklist?"
        ),
    },
    {
        "traceId": "ambiguous-profile-switch",
        "question": (
    ...
    return [
        {
            ...
            "movement": rank_movement(item.previous_rank, item.rank),
            "rrfScore": round(item.rrf_score, 6),
            "denseRank": item.dense_rank,
            "lexicalRank": item.lexical_rank,
            "denseScore": (
...
def main() -> None:
    ...
    for q in QUESTIONS:
        ...
        user_prompt = build_prompt(question, reranked)
        ...
        trace = {
            ...
            "steps": [
                ...
                },
                {
                    ...
                    "detail": {
                        ...
                        "candidateK": settings.candidate_k,
                        "finalContextK": settings.final_context_k,
                        "candidateIds": [item.chunk.id for item in hybrid],
                        "before": [
                            {
                                "id": item.chunk.id,
                                "rank": item.rank,
                        ...
                        "chunks": reranked_detail(reranked),
                    },
                    "metrics": {
                        "latencyMs": rerank_ms,
                        "modelLoadMs": reranker_load_ms,
                        "provenance": "measured",
                    },
                },
                {
                    ...
                    "detail": {
                        ...
                        "finalContextIds": [item.chunk.id for item in reranked],
                    },
                    "metrics": {
                        "latencyMs": 0,
                        "provenance": "measured",
            ...
            "cookbook": {"path": "examples/rag/03-reranking"},
        }
        traces.append(trace)
