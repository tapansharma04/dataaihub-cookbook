"""agent-planning — application-managed multi-step plan runtime.

Example ID: agent-planning

Loop:
  Goal → Create plan → Execute → Observe → Continue / Revise → Final answer
"""
...
def _print_run(result: AgentRunResult, *, show_sequence: bool) -> None:
    ...
    print(
        "Plan: "
        f"v{result.metrics.plan_version} "
        f"status={result.metrics.plan_status} "
        f"revisions={result.metrics.plan_revisions} "
        f"steps={result.metrics.plan_steps} "
        f"(completed={result.metrics.completed_steps} "
        f"skipped={result.metrics.skipped_steps} "
    ...
    if show_sequence:
        ...
        for event in result.sequence:
            if event.kind == "model_decision":
                ...
                print(
                    f"  [{event.kind}] v{event.detail.get('version')} "
                    f"steps={len(event.detail.get('steps') or [])}"
                )
            elif event.kind == "plan_step_started":
                print(
                    f"  [{event.kind}] {event.detail.get('stepId')} "
                    f"v{event.detail.get('planVersion')}"
                )
            elif event.kind == "plan_step_completed":
                print(
                    f"  [{event.kind}] {event.detail.get('stepId')} "
                    f"status={event.detail.get('status')}"
                )
            elif event.kind == "plan_revised":
                print(
                    f"  [{event.kind}] v{event.detail.get('fromVersion')} → "
                    f"v{event.detail.get('toVersion')}"
                )
                ...
                print(
                    f"  [{event.kind}] reason={event.detail.get('reason')} "
                    f"planStatus={event.detail.get('planStatus')}"
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--show-sequence",
        action="store_true",
        help="Print the observable plan/execute/observe sequence",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    try:
        ...
        print(
            ...
            "uv run python main.py --case simple-plan-billing-docs",
            file=sys.stderr,
        )
        return 1
