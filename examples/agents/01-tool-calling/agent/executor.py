...
class ToolExecutor:
    ...
    def execute(self, call: ToolCallRequest) -> ToolCallResult:
        ...
        # Local tools are sync and fast; still enforce a wall-clock bound so
        # the pattern is visible in the example.
        try:
        ...
        # Handlers may return structured soft-failures (unknown entity, etc.).
...
def parse_tool_arguments_json(raw: str) -> dict[str, Any]:
    ...
    Invalid JSON becomes an empty dict plus a sentinel so the executor can
    return invalid_arguments rather than crashing the loop.
    """
    try:
