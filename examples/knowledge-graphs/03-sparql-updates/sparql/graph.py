...
from sparql.model import BindingValue, TripleState, TripleTerm
...
class RdfGraphStore:
    ...
    @classmethod
    def fresh_from_path(cls, path: Path) -> RdfGraphStore:
        """Load a new in-memory graph from Turtle (case isolation)."""
    ...
    def triple_term(self, term: Node) -> TripleTerm:
        if isinstance(term, URIRef):
            return TripleTerm(iri=str(term), label=self.label_for(term))
        if isinstance(term, Literal):
            return TripleTerm(
                iri=str(term),
                literal=str(term),
        ...
        return TripleTerm(iri=str(term))
    ...
    def focused_state(
        ...
        *,
        ...
        """Return normalized triples for optional focus filters."""
        ...
        for subject, predicate, obj in self._graph:
            ...
            triples.append(
                TripleState(
                    subject=self.triple_term(subject),
                    predicate=self.triple_term(predicate),
                    object=self.triple_term(obj),
                )
            )
        triples.sort(key=lambda triple: triple.sort_key())
        return triples
